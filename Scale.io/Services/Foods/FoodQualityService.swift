...
struct FoodQualityService {
    ...
    private let runtime = FoodQualityAIRuntime()

    func requestID(for food: Food) -> String {
        [
            normalizedInlineText(food.name).lowercased(),
            normalizedInlineText(food.brands).lowercased(),
            food.grade?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() ?? "nil",
            food.levels?.fat?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
                ?? "nil",
            food.levels?.saturatedFat?.trimmingCharacters(in: .whitespacesAndNewlines)
                .lowercased() ?? "nil",
            food.levels?.sugars?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
                ?? "nil",
            food.levels?.salt?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
                ?? "nil",
        ].joined(separator: "|")
    }
    ...
    func makeRequest(
        for food: Food,
        hardcoded: FallBackQualityService.Result
    ) -> Request? {
        guard hardcoded.summary != nil else { return nil }

        let prompt = """
            Write a summary for this food item.

            Food name: \(normalizedInlineText(food.name))
            Internal quality grade: \(hardcoded.grade?.rawValue ?? "Unavailable")
            Internal nutrient profile: \(hardcoded.nutrientLevelsDescription)
            Internal main concern: \(hardcoded.topConcernLabel ?? "None")
            """

        return Request(
            key: requestID(for: food),
            prompt: prompt
        )
    }

    func resolve(_ request: Request) async -> String? {
...
private actor FoodQualityAIRuntime {
    private var cache: [String: String] = [:]
    private var isPermanentlyUnavailable = false

    func resolve(_ request: FoodQualityService.Request) async -> String? {
        ...
        let summary = normalizedInlineText(generated)
